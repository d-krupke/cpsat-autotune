...
def tune_for_quality_within_timelimit(model: cp_model.CpModel, timelimit_in_s: float, obj_for_timeout: int, direction: str):
    ...
    for i in range(1, len(best_params)):
        best_params_of_size, diff_to_baseline_of_size, significant_of_size = objective.best_params(i)
        if len(best_params_of_size) == i:
            print_best_params(best_params_of_size, diff_to_baseline_of_size, significant_of_size)
