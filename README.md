...
> CAVEAT: The approach of tuning the hyperparamters on the top-level is dangerous
> and instead one should tune the parameters of a subsolver. The reason is that
> the top-level parameters may be copied to every subsolver including LNS workers.
> If one enables expensive propagation mechanisms and such things, it could seriously
> impact overall solver. I will create an improved version of this tool soon.
